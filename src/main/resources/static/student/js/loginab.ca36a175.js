...
wd.login.init = function () {
    ...
    var a = $("input[name=id]").val(), b = $("input[name=password]").val(), c = $("#auto-login").prop("checked");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var f = b.code;
            0 == f ? ("undefined" != typeof mixpanel && (mixpanel.identify(a), mixpanel.people.set({$id: a})), setTimeout(function () {
                window.location.href = d
            }, 500)) : 1 == f && ($("#login-btn").removeAttr("disabled"), warnings.eq(1).text("学号或密码错误").addClass("warning-password-show"))
        },
        error: function () {
            alert("登录失败\n");
